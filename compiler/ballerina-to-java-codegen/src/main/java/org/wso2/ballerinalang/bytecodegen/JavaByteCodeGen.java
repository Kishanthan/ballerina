...
public class JavaByteCodeGen {
    ...
    public static void registerProgramFile(ProgramFile programFile) throws Exception {
        ...
        for (PackageInfo packageInfo : programFile.getPackageInfoEntries()) {
            if (packageInfo.getPkgPath().startsWith("ballerina/builtin") |
                    packageInfo.getPkgPath().startsWith("ballerina/runtime")) {
                continue;
            }
...
//                    break;
                ...
                case InstructionCodes.CALL:
                    processCallInstr(mv, ctx, consts, (Instruction.InstructionCALL) instr);
                    break;
...
//        FunctionCallCPEntry funcCallCPE = (FunctionCallCPEntry) consts[instruction.funcRefCPIndex];
        ...
        int[] retRegs = instruction.retRegs;
        ...
        private void processCallRetValueMemoryAllocation() {
            ...
            for (int i = 0; i < this.codeLength; i++) {
                ...
                if (instr.getOpcode() == InstructionCodes.CALL) {
                    FunctionInfo callFuncInfo = ((FunctionRefCPEntry)
                            this.getConsts()[((Instruction.InstructionCALL) instr).funcRefCPIndex]).getFunctionInfo();
...
//        Path progDir = Paths.get("/Users/kishanthan/WSO2/msc/samples");
