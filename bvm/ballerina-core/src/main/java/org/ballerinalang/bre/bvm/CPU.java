        ...
        while (ctx.ip >= 0) {
            ...
            switch (opcode) {
                ...
                case InstructionCodes.CCONST:
                    cpIndex = operands[0];
                    i = operands[1];
                    sf.charRegs[i] = ((CharacterCPEntry) ctx.constPool[cpIndex]).getValue();
                    break;
                case InstructionCodes.BTCONST:
                    cpIndex = operands[0];
                    i = operands[1];
                    sf.byteRegs[i] = ((ByteCPEntry) ctx.constPool[cpIndex]).getValue();
                    break;
    ...
    private static void execLoadOpcodes(WorkerExecutionContext ctx, WorkerData sf, int opcode, int[] operands) {
        ...
        switch (opcode) {
            ...
            case InstructionCodes.BTALOAD:
                ...
                try {
                    sf.byteRegs[k] = bByteArray.get(sf.longRegs[j]);
                } catch (Exception e) {
                    ctx.setError(BLangVMErrors.createError(ctx, e.getMessage()));
                    handleError(ctx);
                }
            ...
            case InstructionCodes.CGLOAD:
                i = operands[0];
                j = operands[1];
                sf.charRegs[j] = ctx.programFile.getGlobalMemoryBlock().getCharField(i);
                break;
            case InstructionCodes.BTGLOAD:
                i = operands[0];
                j = operands[1];
                sf.byteRegs[j] = ctx.programFile.getGlobalMemoryBlock().getByteField(i);
                break;
            case InstructionCodes.FGLOAD:
                i = operands[0];
                j = operands[1];
    ...
    private static void execStoreOpcodes(WorkerExecutionContext ctx, WorkerData sf, int opcode, int[] operands) {
        ...
        switch (opcode) {
            ...
            case InstructionCodes.BTASTORE:
                ...
                try {
                    bByteArray.add(sf.longRegs[j], (byte) sf.byteRegs[k]);
                } catch (Exception e) {
                    ctx.setError(BLangVMErrors.createError(ctx, e.getMessage()));
                    handleError(ctx);
                }
            ...
            case InstructionCodes.CGSTORE:
                i = operands[0];
                j = operands[1];
                ctx.programFile.getGlobalMemoryBlock().setCharField(j, sf.charRegs[i]);
                break;
            case InstructionCodes.BTGSTORE:
                i = operands[0];
                j = operands[1];
                ctx.programFile.getGlobalMemoryBlock().setByteField(j, sf.byteRegs[i]);
                break;
            case InstructionCodes.FGSTORE:
                i = operands[0];
                j = operands[1];
    ...
    private static void execBinaryOpCodes(WorkerExecutionContext ctx, WorkerData sf, int opcode, int[] operands) {
        ...
        switch (opcode) {
            ...
            case InstructionCodes.BTEQ:
                ...
                j = operands[1];
                k = operands[2];
                sf.intRegs[k] = sf.byteRegs[i] == sf.byteRegs[j] ? 1 : 0;
                break;
            case InstructionCodes.FEQ:
                i = operands[0];
                j = operands[1];
    ...
    private static void execTypeCastOpcodes(WorkerExecutionContext ctx, WorkerData sf, int opcode, int[] operands) {
        ...
        switch (opcode) {
            ...
            case InstructionCodes.BT2ANY:
                i = operands[0];
                j = operands[1];
                sf.refRegs[j] = new BByte(sf.byteRegs[i]);
                break;
            case InstructionCodes.F2ANY:
                i = operands[0];
                j = operands[1];
            ...
            case InstructionCodes.ANY2BT:
                ...
                if (bRefType == null) {
                    ...
                    sf.refRegs[k] = null;
                    sf.byteRegs[j] = ((BByte) bRefType).byteValue();
                } else {
                    sf.byteRegs[j] = 0;
        ...
        for (int i = 0; i < varRegs.length && lockAcquired; i++) {
            ...
            switch (paramType.getTag()) {
                case TypeTags.INT_TAG:
                    ...
                    break;
                case TypeTags.CHAR_TAG:
                    lockAcquired = ctx.programFile.getGlobalMemoryBlock().lockCharField(ctx, regIndex);
                    break;
                case TypeTags.BYTE_TAG:
                    lockAcquired = ctx.programFile.getGlobalMemoryBlock().lockByteField(ctx, regIndex);
                    break;
                case TypeTags.FLOAT_TAG:
        ...
        for (int i = varRegs.length - 1; i > -1; i--) {
            ...
            switch (paramType.getTag()) {
                case TypeTags.INT_TAG:
                    ...
                    break;
                case TypeTags.CHAR_TAG:
                    ctx.programFile.getGlobalMemoryBlock().unlockCharField(regIndex);
                    break;
                case TypeTags.BYTE_TAG:
                    ctx.programFile.getGlobalMemoryBlock().unlockByteField(regIndex);
                    break;
                case TypeTags.FLOAT_TAG:
        ...
        for (int i = 0; i < regs.length; i++) {
            ...
            switch (paramType.getTag()) {
                case TypeTags.INT_TAG:
                    ...
                    break;
                case TypeTags.CHAR_TAG:
                    result[i] = new BCharacter(data.charRegs[argReg]);
                    break;
                case TypeTags.BYTE_TAG:
                    result[i] = new BByte(data.byteRegs[argReg]);
                    break;
                case TypeTags.FLOAT_TAG:
    ...
    public static void copyArgValuesForWorkerReceive(WorkerData currentSF, int[] argRegs, BType[] paramTypes,
        ...
        for (int i = 0; i < argRegs.length; i++) {
            ...
            case TypeTags.INT_TAG:
                ...
                break;
                case TypeTags.CHAR_TAG:
                    currentSF.charRegs[regIndex] = ((BCharacter) passedInValues[i]).charValue();
                    break;
                case TypeTags.BYTE_TAG:
                    currentSF.byteRegs[regIndex] = ((BByte) passedInValues[i]).byteValue();
                    break;
            case TypeTags.FLOAT_TAG:
